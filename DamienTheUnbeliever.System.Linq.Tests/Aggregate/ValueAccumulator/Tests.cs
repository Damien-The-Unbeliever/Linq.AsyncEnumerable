...
namespace DamienTheUnbeliever.System.Linq.Tests.Aggregate.ValueAccumulator
{
  public class Tests
  {
    [Theory]
    [MemberData(nameof(EarlyValidationCalls))]
    public void Eagerly_Validates(IAsyncEnumerable<int> source, Func<int, int, ValueTask<int>> accumulator)
    {
    ...
    public async Task Empty_Sequence_Throws(IAsyncEnumerable<int> source, Func<int, int, ValueTask<int>> accumulator)
    {
    ...
    public async Task Aggregate_Can_Compute_Factorial(IAsyncEnumerable<int> source, Func<int, int, ValueTask<int>> accumulator)
    {
