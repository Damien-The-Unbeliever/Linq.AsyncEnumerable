...
namespace DamienTheUnbeliever.System.Linq.Tests.Aggregate.Seed.ValueAccumulator
{
  public class Tests
  {
    [Theory]
    [MemberData(nameof(EarlyValidationCalls))]
    public void Eagerly_Validates(IAsyncEnumerable<int> source, Func<int, int, ValueTask<int>> accumulator, int seed)
    {
    ...
    public async Task Empty_Sequence_Yields_Seed(IAsyncEnumerable<int> source, Func<int, int, ValueTask<int>> accumulator, int seed)
    {
    ...
    public async Task Computes_Factorial(IAsyncEnumerable<int> source, Func<int,int,ValueTask<int>> accumulator, int seed)
    {
      var actual = await source.Aggregate(seed, accumulator);
