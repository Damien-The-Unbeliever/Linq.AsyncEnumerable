...
namespace DamienTheUnbeliever.System.Linq.Tests.Aggregate.Seed.ValueSeed
{
  public class Tests
  {
    [Theory]
    [MemberData(nameof(EarlyValidationCalls))]
    public void Eagerly_Validates(IAsyncEnumerable<int> source, Func<int, int, int> accumulator, ValueTask<int> seed)
    {
    ...
    public async Task Empty_Sequence_Yields_Seed(IAsyncEnumerable<int> source, Func<int, int, int> accumulator, ValueTask<int> seed)
    {
    ...
    public async Task Computes_Factorial(IAsyncEnumerable<int> source, Func<int,int,int> accumulator, ValueTask<int> seed)
    {
      var actual = await source.Aggregate(seed, accumulator);
    ...
    public static IEnumerable<object[]> EmptyCalls()
    {
      ...
      {
        call[2] = new ValueTask<int>((int)call[2]);
        yield return call;
      }
    }
