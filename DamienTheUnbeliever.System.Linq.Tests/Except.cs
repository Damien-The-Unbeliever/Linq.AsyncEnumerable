...
namespace DamienTheUnbeliever.System.Linq.Tests
{
  public class Except
  {
    ...
    public void Eagerly_Validates_First_NoComparer()
    {
      ...
      {
        var iter = AsyncEnumerable.Except(null, AsyncEnumerable.Empty<int>());
      });
    }
    [Fact]
    public void Eagerly_Validates_Second_NoComparer()
    {
      Assert.Throws<ArgumentNullException>(() =>
      {
        var iter = AsyncEnumerable.Except(AsyncEnumerable.Empty<int>(),null);
      });
    }
    [Fact]
    public void Eagerly_Validates_First_Comparer()
    {
      Assert.Throws<ArgumentNullException>(() =>
      {
        var iter = AsyncEnumerable.Except(null, AsyncEnumerable.Empty<int>(),new IntComparer());
      });
    }
    [Fact]
    public void Eagerly_Validates_Second_Comparer()
    {
      Assert.Throws<ArgumentNullException>(() =>
      {
        var iter = AsyncEnumerable.Except(AsyncEnumerable.Empty<int>(),null, new IntComparer());
      });
    }

    [Fact]
    public async Task Iteration_Causes_Complete_Iteration_Second_NoComparer()
    {
      ...
      await using (var target = first.Except(second).GetAsyncEnumerator())
      {
        ...
        Assert.Equal(TrackingAsyncEnumerable<int>.Status.EnumerationComplete, second.State);
      }
    }
    ...
    public async Task Iteration_Causes_Complete_Iteration_Second_Comparer()
    {
      ...
      await using (var target = first.Except(second, new IntComparer()).GetAsyncEnumerator())
      {
        Assert.True(await target.MoveNextAsync());
        Assert.Equal(0, target.Current);
        Assert.Equal(1, first.ItemsProvided);
    ...
    public async Task Empty_Second_Yields_First()
    {
      ...
      var target = await first.Except(second).ToList();

      Assert.Equal(10, target.Count);
    }

    [Fact]
    public async Task Non_Empty_Second_Yields_Removes_Elements()
    {
      var first = Enumerable.Range(0, 10).AsAsyncEnumerable();
      ...
      Assert.DoesNotContain(3, target);
    }
  }
}
